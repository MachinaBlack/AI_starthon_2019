    ...
    def create_instances(self, file_path, max_seq_length, type='train'):
        ...
        if type == 'train':
            for label in train_set:
                if len(train_set[label]) < 700:
                    iterate_num = int(700 / len(train_set[label]))
                    for i in range(iterate_num):
                        for sent in train_set[label]:
                            text = sent[0]
                            intent = sent[1]
