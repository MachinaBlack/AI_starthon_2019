...
class BIMPM(nn.Module):
    def __init__(self, config, feature):
        ...
        self.d = config.word_dim + int(config.use_char_emb) * config.char_hidden_size
        self.l = config.num_perspective

        # ----- Word Representation Layer -----
        self.char_emb = nn.Embedding(config.char_vocab_size, config.char_dim)

        self.word_emb = nn.Embedding(config.vocab_size, config.word_dim, padding_idx=Vocabulary.PAD_IND)
        # TODO: check after change this to True
        # no fine-tuning for word vectors
        self.word_emb.weight.requires_grad = True

        self.char_LSTM = nn.LSTM(
            input_size=config.char_dim,
        ...
        for i in range(1, 9):
            setattr(self, f'mp_w{i}',
                    nn.Parameter(torch.rand(self.l, config.hidden_size)))
        ...
        # ----- Prediction Layer -----
        self.pred_fc1 = nn.Linear(config.hidden_size * 4, config.hidden_size * 2)
        self.pred_fc2 = nn.Linear(config.hidden_size * 2, config.class_size)
    ...
    def forward(self, questions1, questions2, features, is_train=True):
        ...
        p = self.word_emb(questions1)
        h = self.word_emb(questions2)
        ...
        _, tmp_idx = att_fw.max(dim=2)
        att_max_h_fw_idx = torch.stack([tmp_idx]*self.config.hidden_size, dim=2)
        _, tmp_idx = att_bw.max(dim=2)
        att_max_h_bw_idx = torch.stack([tmp_idx]*self.config.hidden_size, dim=2)

        att_max_h_fw = torch.gather(con_h_fw, dim=1, index=att_max_h_fw_idx)
        att_max_h_bw = torch.gather(con_h_bw, dim=1, index=att_max_h_bw_idx)

        # (batch, seq_len2, hidden_size)
        _, tmp_idx = att_fw.max(dim=1)
        att_max_p_fw_idx = torch.stack([tmp_idx]*self.config.hidden_size, dim=2)
        _, tmp_idx = att_bw.max(dim=1)
        att_max_p_bw_idx = torch.stack([tmp_idx]*self.config.hidden_size, dim=2)

        att_max_p_fw = torch.gather(con_p_fw, dim=1, index=att_max_p_fw_idx)
        att_max_p_bw = torch.gather(con_p_bw, dim=1, index=att_max_p_bw_idx)
